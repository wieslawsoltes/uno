...
| Uno forward gradient local-matrix adaptation | native C++ brush records already consume the same inverse destination-to-brush coordinate transform as the managed renderer; the conversion belongs to each framework adapter, not either renderer | focused inverse/singular adapter contract plus managed/native shared-shader gates |
